...
def get_component_cmakelists() -> str:
    ...
    # Collect custom CMake code registered by components (e.g. ULP firmware
    # compilation via ulp_embed_binary()). This is appended after
    # idf_component_register() so the component library target exists.
    extra_cmake = "\n".join(CORE.data[KEY_ESP32][KEY_EXTRA_CMAKE])
...
# Exclude ULP source files — they are compiled by the ulp_embed_binary()
# function below, not the main compiler.
list(FILTER app_sources EXCLUDE REGEX "/ulp/.*$")

idf_component_register(
    SRCS ${{app_sources}}
    INCLUDE_DIRS "." "esphome"
    REQUIRES ulp ${{ESPHOME_PROJECT_BUILTIN_COMPONENTS}}
)
